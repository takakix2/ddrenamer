...
**ディレクトリ境界**として読む（＝リネームではなく移動になる）。`ＣＯＮ` も同じで、半角化すると
デバイス名になる。**その番人が、実際にそれらを拒否する OS の上で動くのを見たのは今日が初めて**。
2026-08-01 に立てた `check_portable_name` は、ext4 と CI の中でしか走っていなかった。
