...
最終更新: 2026-07-31（blackcube → **m4air へ引き継ぎ中**）
直近の変更: **Undo を実装**（バッチ単位・`Ctrl+Z`・ログバーのボタン）＋
...
## 🎯 次にやること — **m4air（macOS）で手を動かす番**

現行コードは **macOS で一度も操作されていない**。2026-07-31 に `71408fc` から焼き、
**起動して 41 秒生存・CPU 0.0%・クラッシュ無し**までは確認したが、**そこから先は
ssh から触れない**（補助アクセス TCC が無く `osascript` が `-1728` で拒否される）。

### 🔴 実機で確かめてほしいこと 3 つ

1. **UI が操作できるか**（最重要）。2026-07-29 に `1d89a15` で
   「描画は完全なのに**全操作が無反応**」という不具合が出ている（原因は `decorations: false`・
   下の解決済みセクション）。窓を掴む / タブを切り替える / 文字を打つ、が通るか
2. **入力欄で `⌘C` / `⌘V` / `⌘X` / `⌘Z` が効くか** ← **この答えが Linux 側の判断を決める**
   - このアプリは**ネイティブメニューを 1 つも構築していない**（`lib.rs` に menu 無し）。
     Tauri の macOS アプリはメニューが無いと `⌘C`/`⌘V` すら効かないのが通例
   - Linux では **`Ctrl+Z` だけが効かない**ことが実測で分かっている（下の follow-up）。
     macOS でも同じなら「WebKit 全般の話」、macOS では効くなら「メニューで直る話」になり、
     **Linux に自前 undo スタックを作るかどうかが決まる**
3. **Undo が動くか** —— ログバー右の ↶ ボタンと `⌘Z`。リネームして戻して、
   名前と中身が復帰するか（Linux では実機確認済み）

### 📌 ついでに見えるもの

- **リサイズハンドルを macOS で描いていない判断**（`decorations: true` ＝ ネイティブの縁が在る
  という読み）。`titleBarStyle: Overlay` との兼ね合いは未検証なので、**縁を掴んでみること**
- **`⌘,`** で設定モーダルが開くか（キーは実装済み・**メニュー項目は無い**）

⚠️ **配布はまだできない。** `spctl -a -vv` は
...
| macOS aarch64 | **m4air** | ✅ **2026-07-31 に `71408fc` で焼き直し済み**（番人 + Undo 入り）: `DDRenamer.app 17M` / **`dmg 9.4M`**（前回は dmg 未生成）。arm64。⏸ **操作確認は未了** |
...
⏸ **操作確認は未了。** ssh セッションには**補助アクセス（TCC）が無い**ので
`osascript` が `-1728` で拒否される。⇒ 実機で手を動かすしかない（上の「次にやること」）。
...
- 🔶 **macOS 版の焼き直しと配布判断。** UI 無反応は直ったので配れる状態になったが、
  **署名は adhoc のまま**（次項）。`--bundles app` でしか焼いていないので **dmg は未生成**。
  ...
  ⏸ **release ビルドでの動作確認はしていない**（ユニットテストと配線の検証まで）。
...
- 🔶 **入力欄の `Ctrl+Z` / `Ctrl+Shift+Z` が効かない**（2026-07-31 実測・原因を切り分け済み）。
  ⚠️ このアプリは**ネイティブメニューを 1 つも構築していない**（`lib.rs` に menu 無し）。
  ...
  **直し方の見込み**:
  - **macOS**: Edit ロール付きのネイティブメニューで直る見込み（WKWebView は undo manager を持つ）。
    しかも `⌘C`/`⌘V` はメニューが無いと**そもそも効かない**のが通例なので、macOS では必須。
    「**設定… `⌘,`** のメニュー項目が無い」という積み残しと**同じ作業**なのでまとめてやること
  - **Linux**: ⚠️ **メニューでは直らない見込み**。undo ロールは webview の undo に流れるが、
    そこが無い。直すなら**自前の undo スタック**を欄ごとに持つことになる（実装量に対して
    見合うかは要判断）。
    🔴 **判断は macOS の実測待ち**（2026-07-31 時点）。macOS で `⌘Z` が効くなら
    「メニューで直る話」＝ Linux だけの制約として受け入れる選択肢が立つ。
    macOS でも効かないなら WebKit 全般の話なので、自前で作るしかない
