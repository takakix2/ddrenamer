...
最終更新: 2026-08-19（腐っていた 4 行を直しただけ。**コードは 2026-08-01 から動いていない**）
前回の実作業: 2026-08-01（**3 プラットフォーム全部が焼けるようになった日**。
macOS = 実機確認 + 署名/notarize 済み / Linux = release で操作確認済み /
**Windows = CI で初めて焼けた**（msi + nsis exe・⏸ 誰も触っていない））

---

## 🚦 次に開くならここ（2026-08-01 の終わりの状態）

**コードの宿題は一巡した。** 残りは性質が違う 3 つだけ:

| # | 何 | どこで |
|---|---|---|
| **1** | 🪟 **Windows を実機で触る** —— msi/exe は焼けたが**誰も操作していない**。自前 CSD なので macOS の `1d89a15`（描画は完全なのに全操作が無反応）と同じ事故が起こりうる。**最初に窓を掴む・タブを押す・文字を打つ** | ✅ **機械は在る**: `dyna` (192.168.1.9 / Win11 Pro 25H2)。2026-08-19 に LAN で確認 |
| **2** | 🍎 **macOS の成果物が古い** —— 署名 + notarize が通っているのは 06:5x の版で、同日午前の分（入力欄 undo / 名前の検査 / ライセンス同梱）が**入っていない**。配る前に**焼き直し → 再署名 → 再 notarize → staple** | m4air |
| **3** | 🔶 **配布先の決定**（GitHub Releases が自然か） | 判断 |
...
**焼くのは解決した。残っているのは「触ること」だけ。**

| | 状況 |
|---|---|
| 焼ける機械 | ✅ **CI で焼ける**（GitHub Actions `windows-latest`・下の「Windows: CI を置いた」） |
| CI | ✅ **在る**（`.github/workflows/windows.yml`・緑 `30680237956`） |
| 触れる機械 | ✅ **`dyna`（192.168.1.9・Windows 11 Pro 25H2）が LAN に居る**（2026-08-19 に確認） |
| 設定 | ✅ `bundle.targets: "all"` なので設定上は焼ける |
| コード | ✅ `cfg(target_os)` 分岐ゼロ / フロントの OS 分岐も `isMac` だけ |
| 名前の検査 | ✅ **実装済み**（常に Windows の規則で弾く・下の「実装: 置けない名前を弾く」） |

🚨 **2026-08-19 に上の表を 3 行書き換えた。** ここは長く「Windows 機が無い / CI が無い /
名前の検査が無い」と言っていたが、**3 つとも既に解消していた**（CI と名前の検査は 2026-08-01 に
同じファイルの別セクションで閉じ、`dyna` は後から LAN に生えた）。
⭐ **宿題の欄は、宿題でなくなったことを自分では知らない。**

**形としては合っている見込み**: Windows は Linux と同じ経路（`decorations: false` + 自前の
窓操作）に乗る。− □ ✕ を右上に描くのは Windows の作法と一致するので、CSD はむしろ素直。

✅ **「名前として置けない文字」の検査は 2026-08-01 に入った**（`check_portable_name`）。
禁止文字 `< > : " / \ | ? *`・制御文字・予約名（`CON` `PRN` `AUX` `NUL` `COM1-9` `LPT1-9`）・
末尾のドット / 空白を、**OS で切り替えず常に Windows の規則で**弾く（判断の理由は下の
「実装: 置けない名前を弾く」）。⇒ **Windows で初めて弾かれる名前、は原理上もう出ない。**

🚨 **だから実機で見るべきものは「名前」ではなく「窓」。** 自前 CSD（`decorations: false` +
自前の − □ ✕）なので、macOS の `1d89a15`（**描画は完全なのに全操作が無反応**）と同じ事故が
Windows でだけ起こりうる。**最初に窓を掴む・タブを押す・文字を打つ** —— 見た目で判断しない。
