...
最終更新: 2026-08-19（**3 プラットフォーム全部が実機で触られた日**。
macOS = 実機確認 + 署名/notarize 済み / Linux = release で操作確認済み /
**Windows = `dyna` で焼いて触った**。⏸ 残るは macOS の焼き直しと配布先だけ）

---

## 🚦 次に開くならここ（2026-08-19 の終わりの状態）

**残りは 2 つ。どちらもコードを書かない仕事。**

| # | 何 | どこで |
|---|---|---|
| **1** | 🍎 **macOS の成果物が古い** —— 署名 + notarize が通っているのは 2026-08-01 06:5x の版で、同日午前の分（入力欄 undo / 名前の検査 / ライセンス同梱）**も、2026-08-19 の分（Case / Convert のタブ）も**入っていない。配る前に**焼き直し → 再署名 → 再 notarize → staple** | m4air |
| **2** | 🔶 **配布先の決定**（GitHub Releases が自然か）。⚠️ **CI は GitHub 側にしか無い**ので、CI を使うこと自体が公開を意味する。`dyna` で焼けるようになった今、その 2 つは切り離せる | 判断 |

✅ **Windows を実機で触る —— 2026-08-19 に完了**（下の「Windows: 実機で触った」）。
⏸ **Linux は 2026-08-01 の release までしか触っていない** —— Case / Convert のタブは
**Windows でしか実機確認していない**。同じコードなので通る見込みだが、見てはいない。

⏸ **見ていないと分かっているもの**（見ていないことを書くのも成果物）:
- `rpm` と macOS `.app` にライセンスファイルが載っているか（deb / AppImage は確認済み）
- 入力欄 redo が 1 度だけ戻らなかった件の再現条件

📌 **潜る前に `git fetch` すること。** このリポは 2 台で進む。2026-08-01 は blackcube が
**3 コミット遅れのまま**作業を始めて、古い前提で報告しかけた。
📌 **m4air で pull する前に注意** —— 2026-08-01 に**履歴を書き換えた**（公開前の伏せ字）。
普通の `git pull` だと古いコミットが戻るので `git fetch origin && git reset --hard origin/main`。
（m4air は同日に適用済み。3 台目が在れば同じことが要る）
...
同日: **Windows CI を新設**し、その過程で**このリポが fresh clone からビルドできない**ことが判明
（`src/ui-kit` の symlink・下の専用セクション）。**LICENSE を MIT に確定**し、
OFL の第三者通知を**配布物に載せた**。
同日: **macOS 実機で全項目が通った**（下の「macOS 実機確認」）。
...
- ✅ **LICENSE は MIT に確定**（同日・下の「決定: ライセンスは MIT」）。`LICENSE` も
  README も第三者通知も在る。⚠️ この行は 2026-08-19 まで「未決・ファイルも無い」と
  言い続けていた —— **同じ日に閉じた宿題が、閉じたことを知らない欄に残っていた**。
- 🔶 **配布先が未決**（`.github/FUNDING.yml` だけ在るので GitHub Releases が自然か）。
- **設定… `⌘,`** のメニュー項目（キーは効く・項目が無いだけ。上の 🚨 を読んでから）

---

## ✅ 片付いた宿題（2026-08-01 に全部閉じた）

**未了の一覧は先頭の「次に開くならここ」に 1 つだけ置く。** ここは閉じた物の索引:

| 何 | どこに書いてあるか |
|---|---|
| 入力欄の `Ctrl+Z`（OS 間の非対称） | 「解決: 入力欄の `Ctrl+Z`」 |
| Linux release での動作確認 | 「リリース成果物」 |
| 置けない名前の検査（常に Windows の規則で弾く） | 「実装: 置けない名前を弾く」 |
| 空の名前で隠しファイルが作れる | 「修正済: 名前の番人を `join_name_ext` に集約」 |
| 大小だけ違うファイルを黙って上書き | 「修正済: 大小だけ違うファイル…」 |
| LICENSE（MIT）と第三者通知 | 「決定: ライセンスは MIT」 |
| Windows を焼く | 「Windows: CI を置いた」 |
...
## ✅ Windows: 実機で触った（2026-08-19・`dyna`）

**全部通った。** 「マルチ OS で作っていた」が、初めて 3 つとも実機で確かめられた状態になった。

| 見たもの | 結果 |
|---|---|
| 窓を掴んで動かす（自前 CSD） | ✅ 動く |
| − □ ✕ | ✅ 効く |
| タブ 2 行 × 4 の切替 | ✅ 出る・切り替わる |
| 実行ログ | ✅ 見える |
| `Ｐｈｏｔｏ　０１.JPG` →[全角/半角:半角] | ✅ `Photo 01.JPG` |
| `IMG_002.JPG` →[ケース変換:小文字] | ✅ `img_002.JPG`（`.JPG` は大文字のまま＝仕様どおり） |
| `ａ／ｂ.jpg` →[半角] | ✅ **拒否**（`Invalid character '/'`）・名前は不変 |
| `ＣＯＮ.txt` →[半角] | ✅ **拒否**（`Reserved name 'CON'`）・名前は不変 |

🚨 **恐れていた事故は起きなかった。** macOS の `1d89a15`（描画は完全なのに全操作が無反応）と
同じ自前 CSD の経路なので Windows でも疑っていたが、**掴めるし押せる**。⇒ **CSD が壊れるのは
「自前だから」ではなく macOS 固有の事情だった**（あちらは `decorations: false` が犯人で、
今は macOS だけ `decorations: true` にしてある）。Linux と Windows は同じ経路で素直に動く。

⭐ **下 2 行がこの日の一番の収穫。** 半角化は、この道具で唯一「使い手が打っていない文字を
**発明できる**」変換 —— `／`(U+FF0F) は名前として合法なのに、半角の `/` は `parent.join()` が
**ディレクトリ境界**として読む（＝リネームではなく移動になる）。`ＣＯＮ` も同じ。
2026-08-01 に立てた `check_portable_name` は、それまで ext4 と CI の中でしか走っていなかった。

### 🚨 そして `CON` の方は、根拠が間違っていた（同日・実測して判明）

`ＣＯＮ.txt` が弾かれるのを見て「Windows が拒否する名前だから正しい」と書きかけたが、
**Windows 11 (build 26200) で測ったらそうではなかった**:

| | Rust `fs::rename` で作れるか | 作った後 cmd で扱えるか |
|---|---|---|
| **`CON.txt`** | ✅ 作れる | ✅ **`type` / `copy` / `del` 全部通る＝普通のファイル** |
| **`CON`**（裸） | ✅ 作れる（ディスクに載る） | ❌ **届かない**（`type CON` はコンソールを読みに行って固まる） |

⇒ **装置なのは裸の名前だけ。`CON.txt` は今の Windows では普通のファイル。**
「the portable-name guard refuses what Windows refuses」は**この Windows については嘘**だった。

**それでも弾く方を選んだ**（2026-08-19 に判断）。古い Windows では `CON.txt` も装置に解決され、
Microsoft も今なおこの形を避けるよう書いている。⭐ **この番人が答えるべき問いは
「目の前の OS が受け付けるか」ではなく「渡った先で開けるか」**（LAN には SMB 共有と
複数世代の機械が在る）。⚠️ **代償は幅変換が払う** —— `ＣＯＮ.txt` はこの道具では正規化できない。

📌 **README に「作れない名前」の節を作った**（使い手向け・2026-08-19 に英日 2 枚に分けたので `README.md`(en) と `README.ja.md` の両方に在る。⚠️ 片方だけ直すと必ずもう片方が腐る）。⏭ アプリ内にヘルプを持つなら、
**変換できない名前としてそこにも書く**こと（2026-08-19 の申し送り）。
⭐ **弾く判断そのものより、弾く根拠を偽らないことの方が壊れやすい。**
「OS が拒否する」と書くと、OS が変わった日に嘘になり、しかも誰も測り直さない。

### `dyna` の支度（2026-08-19 に整えた）

| | |
|---|---|
| Rust 1.97.1 `stable-x86_64-pc-windows-msvc` | 元から在った |
| Visual Studio Build Tools 2022 | 元から在った |
| git | 元から在った |
| **bun 1.3.14** | ⬅ **この日入れた唯一の物** |

**焼く**: `bun install && bun run tauri build` —— **4 分 08 秒**で `ddrenamer.exe` (16.5MB) +
msi (8.9MB) + nsis setup.exe (7.7MB)。WiX と NSIS は tauri-cli が自分で落としてくる。
📌 **触るだけなら `target\release\ddrenamer.exe` を直接叩くのが早い**（インストール不要・
アンインストールの手間も無い）。

🚨 **`dyna` は Gitea から pull できない。** host key は入れたが、今度は Windows 側の
`known_hosts` の ACL で `Permission denied` になって読めない。深追いせず、blackcube から
**`.git` だけを tar で送って `git reset --hard`** した（4.2MB）。⇒ **今の `dyna` は
「blackcube から配られた checkout」で、更新のたびに送り直しが要る。**
常用するなら ACL と Gitea の鍵登録を別途片付けること。

🚨 **SSH から GUI は上げられない。** Windows の sshd は対話デスクトップとは**別のセッション**を
作るので、そこで起動しても**人の画面には窓が出ない**（見えない机に描かれる）。
⇒ **焼くのは SSH から、触るのは実機の画面から**。CI のコメントが
`the shell still needs a pair of hands` と書いているのはこの構造のこと。

🚨 **`.ps1` を送るときは UTF-8 BOM を付ける。** PowerShell 5.1 は BOM 無し UTF-8 を
**cp932 として読む**ので、日本語を含むスクリプトが壊れて変数が null になる（実際に踏んだ）。
⚠️ SSH 越しの表示が化けるのは**コンソールが cp932 だから**で、ディスク上の名前は正しい。
**化けた表示でファイル名の正否を判断しないこと** —— 確かめるならコードポイントを出させる。

📌 **`.github/workflows/windows.yml` の先頭コメントが古くなった** ——
「There is no Windows machine on the LAN, so until one arrives this is the only place the
guards get exercised where they matter」。**`dyna` が来た。** CI の役目は
「クリーンな checkout でビルドが再現すること」と「push のたびに Windows FS でテストが走ること」に
絞られる（それは残す価値がある）。

### 2026-08-01 に立てた 2 つの予想 —— 答え合わせ

| 予想 | 結果 |
|---|---|
| 「Windows は Linux と同じ経路に乗るので、CSD はむしろ素直」 | ✅ **当たり**。− □ ✕ も掴みも効いた |
| 「実機で見るべきは名前ではなく窓。名前は `check_portable_name` が既に塞いだ」 | ✅ **当たり**。名前は 1 件も想定外を出さず、窓も無事だった |

⇒ **予想が両方当たったので、この日の実機確認は「何も見つからなかった」。**
それ自体が成果物なので消さずに残す —— 次に Windows を疑う人が、
**どこを既に見たのか**を知らずに同じ所を探し直さないために。

📌 `tauri.windows.conf.json` を足す場合は、macOS 側と同じ
**JSON Merge Patch の配列丸ごと置換**の罠を踏む（下の該当セクション参照）。

## ✅ 実装: `Case` と `Convert` に入口を作った（2026-08-19）

**Rust は 1 行も書いていない。** 2 つとも**実装は終わっていて、UI から到達できなかった**。

| | 数 |
|---|---|
| `RenameCommand` の列挙子 | **8**（`Fixed` `Serial` `Replace` `Add` `Trim` `Extension` **`Case`** **`Convert`**）|
| フロントが送っていた `mode` | **6**（`"Case"` / `"Convert"` は `src/` のどこにも無かった）|
| `<TabButton>` | **6** |
| `test_rename_*` | **6 モード分**（= タブと同じ数）|

🚨 **要件定義書は両方を「✅ 完了」と書き続けていた。** Rust だけ見れば嘘ではない。
**使い手から見ると存在しなかった。**

### なぜ誰も気づかなかったか —— **どちらのコンパイラの管轄でもない**

- **Rust**: `Case` / `Convert` は `handle_rename` の `match` に**確かに書いてある**。だから
  `to_zenkaku` / `to_hankaku` も「使われている関数」に見え、**`dead_code` 警告は出ない**
- **TypeScript**: 送るのは `mode: "Fixed"` という**ただの文字列**。書かなかった 2 つは
  TS からは**存在すら見えない**
- 継ぎ目は `#[serde(tag = "mode")]` の **JSON 文字列**。「Rust が受ける 8 と TS が送る 6」を
  突き合わせる者が**誰もいない**

⭐ **型安全な両端を、型のない一点で繋いだ。両側が緑でも、真ん中は誰も見ていない。**

📌 **テストが同じ穴を同じ形で持っていた。** `test_rename_*` は**タブに対して**書かれていて、
enum に対して書かれていなかった。⇒ **配線の欠けが、テストの欠けを隠した**（どちらも
「6 つある」ので、互いを指し示せない）。⏭ **`RenameMode` と `RenameCommand` の一致を
機械で見る検査**を `check:locales` と同じ棚に置くべき（未了）。

### 決めたこと

| 論点 | 決定 |
|---|---|
| タブの並び | **2 行 × 4**（3 行にしない）。ドロップゾーンの高さを削らない方を採った ——「広い」がこの道具の芯 |
| CSS | **無変更**。`.mode-tab` が既に `flex: 1` なので 4 等分される |
| パネルの形 | TRIM と同じ `<Trans>` の一文。ja は選択子が文中（「名前を [小文字] にする」）、en は文末 ⇒ **並びは locale が決める** |
| 拡張子 | **変えない**。`.JPG` を `.jpg` にするのは拡張子タブの仕事。地の文（「拡張子はそのまま」）でそう言う |

### テスト 9 本（**22 → 31**・配線より先に書いた）

⭐ **半角化は、この道具で唯一「使い手が打っていない文字を発明できる」変換。**
`／`(U+FF0F) `：` `＼` `？` は名前として合法なのに、半角の双子は `check_portable_name` が
禁じている当のもの。特に `／`→`/` は `parent.join()` が**ディレクトリ境界**として読む
（＝リネームではなく**移動**）。だから番人は「人が書いていない名前」の上で効く必要がある。

**9 本とも一発で通った** ＝ 新しいバグは無く、2026-08-01 の番人が誰も走らせたことのない経路でも
既に効いていた。テストはそれを**固定**しただけ（番人を `join_name_ext` に戻すとここが落ちる）。

### 実機確認の場所が Linux ではなく Windows になった

🚨 **blackcube のクリップボード経路が死んでいる。** HANDOFF が 2026-07-31 に確立した
「`xclip -t text/uri-list` に積んで `Ctrl+V`」が**無反応**（`hasFiles()` が false）。
形式は合っている（`clipboard-rs` の `FILE_LIST` = `text/uri-list` を読んで確認）ので、
Wayland ↔ XWayland のクリップボード所有の競合が疑わしい。⚠️ `wl-clipboard` は未導入。
📌 **切り分けは既存タブでやった** —— 「リネーム」タブでも同じく無反応 ⇒ **ハーネス側**であって
新しい配線のせいではない。⭐ **新しい物が動かないときは、まず古い物で同じことをやる。**

⇒ タブ →`cmd` 生成 →`invoke` の一本道は、**`dyna` の実機 D&D で通した**（上の Windows の節）。
...
- 🔶 **配布判断**（先頭の「次に開くならここ」の #2 / #3 と同じ話。**未了はあちらが正**）。
  ✅ 操作確認まで済んでいるのは **macOS と Linux の 2 つ**
  （macOS: 2026-08-01 早朝・m4air / Linux: 同日 09:4x・blackcube）。
  ⏸ **Windows は焼けただけで誰も触っていない**（CI・msi + nsis）。
  🚨 **そして macOS の成果物は古い。** 署名 + notarize が通っているのは **06:5x に焼いた版**で、
