...
| プラットフォーム | 焼いた場所 | 成果物 |
|---|---|---|
| Linux x86_64 | blackcube | ✅ **2026-07-31 08:32–08:35 に焼き直し済み**（番人の修正入り）: `deb 11M` / `rpm 11M` / `AppImage 83M` |
...
- [ ] **Undo** — 設計は決定済み（下の専用セクション）。実装は未着手
- [ ] **サウンド通知** — 完了時の SE
- [ ] **多重拡張子対応** — `.tar.gz` などの正確なパース

## 🔙 Undo — 設計決定（2026-07-31・実装は未着手）

**なぜ要るか**: この道具は速く投げられる ＝ **速く間違えられる**。今日 2 件のバグを直したが、
どちらも **Undo が在れば事故にならなかった**。番人を増やすより効く場面がある。
...
`logbar.undo`（ボタンの title）/ 結果の要約行。⚠️ **ログの status は英語のレーン**なので、
Rust が返す失敗理由は翻訳しない（既存の規約どおり）。
