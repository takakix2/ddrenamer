...
最終更新: 2026-08-01（**3 プラットフォーム全部が焼けるようになった日**。
...
| # | 何 | どこで |
|---|---|---|
| **1** | 🪟 **Windows を実機で触る** —— msi/exe は焼けたが**誰も操作していない**。自前 CSD なので macOS の `1d89a15`（描画は完全なのに全操作が無反応）と同じ事故が起こりうる。**最初に窓を掴む・タブを押す・文字を打つ** | 実機待ち |
