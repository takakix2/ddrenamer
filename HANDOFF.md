...
一度も動かしていない**ので、焼き直しは「配るため」だけでなく**動作確認そのもの**でもある。
...
UI 移行・i18n・窓操作は Linux でしか見ていない）。
...
- 🔶 **入力欄の編集ショートカットが半分しか効かない**（2026-07-31 に release ビルドで実測）。
  ...
  | `Ctrl+X` 切り取り | ❌ 効かない |
  | `Ctrl+Z` 元に戻す | ❌ 効かない |
  | `Ctrl+Shift+Z` やり直し | ❌ 効かない |

  **境目は「欄の中身を webview 側が直接書き換える操作」**。読むだけ（選択・コピー）と、
  React が拾う input イベントを伴う貼り付けは通り、**切り取り・元に戻す・やり直しだけが落ちる**。
  📌 **仮説（未検証）**: 入力が controlled（`value={serialText} onChange={...}`）なので、
  WebKit が DOM の値を書き換えても React が知らず、**次のレンダーで state の値に戻される**。
  検証するなら uncontrolled な欄を 1 つ置いて同じ操作を試すのが早い。
  ⚠️ **アプリの Undo（`Ctrl+Z`）とは無関係** —— あちらは入力欄では意図的に何もしない
  （素通しする）ので、原因ではないことは確認済み。

  🚨 **macOS はもっと悪い可能性が高い。** Tauri の macOS アプリは**アプリメニューが無いと
  `⌘C`/`⌘V` すら効かない**のが通例で、この状態の macOS 実機確認は**一度もしていない**。
  ⇒ **Edit ロール付きのネイティブメニューを組む**のが本筋で、それは
  「**設定… `⌘,`** のメニュー項目が無い」という積み残しと**同じ作業**。まとめてやること。
